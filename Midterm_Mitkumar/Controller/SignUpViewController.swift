...
class SignUpViewController: UIViewController {
    ...
    @objc func emailCheck() {
        ...
        if isValidEmailAddress(emailAddressString: email.text!) {
            let imageView = UIImageView(frame: CGRect(x: 0, y: 0, width: email.frame.width, height: email.frame.height))
            imageView.image = UIImage(named: "Path 1939")
            imageView.contentMode = .scaleAspectFit
            email.rightView = imageView
    ...
    override func viewDidLoad() {
        ...
        let paddingView2: UIView = UIView(frame: CGRect(x: 0, y: 0, width: 20, height: username.frame.height))
        ...
        email.backgroundColor = UIColor.groupTableViewBackground // Use anycolor that give you a 2d look.

        //To apply corner radius
        email.layer.cornerRadius = 10

        //To apply border
        email.layer.borderWidth = 0.25
        email.layer.borderColor = UIColor.white.cgColor

        //To apply Shadow
        email.layer.shadowOpacity = 1
        email.layer.shadowRadius = 3.0
        email.layer.shadowOffset = CGSize.zero // Use any CGSize
    ...
    @IBAction func validation(_ sender: Any) {
        ...
        if (isNameValid && isEmailValid && isPasswordValid) {
            ...
            displayAlertMessage(messageToDisplay: "Please provide valid Email Address!")
        }
        else if (!isNameValid) {
